...
public class SourceFilter extends FilterFramework
{
	public void run()
    {
		...
		InputStream input1; 
		InputStream input2;
		byte databyte1 = 0;					// The byte of data read from the file
		byte databyte2 = 0;
		...
		try
		{
			...
			***********************************************************************************/
			...
			while(true)
			{
				databyte1 = in1.readByte();
				databyte2 = in2.readByte();
				bytesread++;
				WriteFilterOutputPort(databyte1);
				WriteFilterOutputPort(databyte2);
				byteswritten++;
